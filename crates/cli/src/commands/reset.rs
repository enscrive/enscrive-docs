//! `enscrive-docs reset` — delete-and-rebuild a documentation corpus.
//!
//! `/v1` has no bulk "delete all documents" endpoint, so reset is implemented
//! as a full corpus delete followed by a recreate (via the same bootstrap
...
pub struct ResetArgs {
    /// Limit to a single corpus by name. Default: reset every corpus
    /// in the config.
    #[arg(long)]
    pub corpus: Option<String>,

    /// Skip the re-ingest step after the corpus is recreated
    #[arg(long)]
...
pub async fn run(global: GlobalArgs, args: ResetArgs) -> Result<(), String> {
    if !args.yes {
        return Err(
            "`reset` deletes and recreates corpora (embeddings are lost). \
    ...
    for corpus_cfg in &cfg.corpora {
        if let Some(only) = args.corpus.as_deref()
            && corpus_cfg.name != only
        {
            continue;
        }
        targeted += 1;
        let Some(existing_entry) = existing.iter().find(|c| c.name == corpus_cfg.name) else {
            println!(
        ...
        println!("[{}] deleted", corpus_cfg.name);
    }
    ...
    println!("reset: re-running bootstrap to recreate corpora");
