...
- set the `path` on `[[corpora]]` to your markdown directory
- set `embedding_model` on `[[corpora]]` (e.g. `"openai/text-embedding-3-small"`) so
  `bootstrap` can create the corpus if it doesn't already exist
...
corpus has enough content to filter against. See [Voices](/voices) for the full
...
One command creates the voice, creates the corpus, and runs the first ingest. It's
idempotent — re-running against an existing tenant skips any voice or corpus that
already exists and just re-ingests. Pass `--skip-ingest` if you only want to provision
the voice + corpus without pushing content yet.
...
- `enscrive-docs reset --yes` deletes and recreates the corpus (then re-runs
